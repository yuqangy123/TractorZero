...
'''
def get_observation_state(major, level_point, curr_score, banner_position, my_position, my_hold_cards, all_player_play_card_history):
    
    major_data = np.zeros(len(__MAJOR__))
    major_data[__MAJOR__.index(major)] = 1
    
    level_point_data = np.zeros(len(__POINT__))
    level_point_data[__POINT__.index(level_point)] = 1
    
    #分数超过160就是顶格了
    curr_score_data = np.zeros(32)
    curr_score_data[:curr_score//5] = 1
    
    banner_position_data = np.zeros(__PLAYER_COUNT__)
    banner_position_data[banner_position] = 1
    
    my_position_data = np.zeros(__PLAYER_COUNT__)
    my_position_data[my_position] = 1
    
    my_hold_cards_data = np.zeros(__CARDS_NUM__)
    my_hold_cards_data[my_hold_cards] = 1
    
    played_cards_data = np.zeros(__CARDS_NUM__)
    player_history_data = np.zeros(__PLAYER_COUNT__)
    for i, history in enumerate(all_player_play_card_history):
        played_cards_data[history] = 1
        player_history_data[i] = np.zeros(__CARDS_NUM__)
        player_history_data[i][history] = 1
        
    ret = np.concatenate([major_data, level_point_data, curr_score_data, banner_position_data, my_position_data, my_hold_cards_data, played_cards_data, player_history_data])
    return ret
...
class tractorGame():
    ...
    def checkResP(self, poker, own, level): # poker: list[int]
        pok = [self.num2Poker(p) for p in poker]
        own_pok = [self.num2Poker(p) for p in own]
        if pok[0] in self.Major:
            major_pok = [pok for pok in own_pok if pok in self.Major]
                    ...
                    for i in range(len(pos)-1):
                        if pos[i+1]-pos[i] == 1:
                            ...
                            if tmp >= len(poker)/2:
                                out = []
                                for j in range(i+2):
                                    out.extend([self.Poker2Num(pok[0][0]+__cardscale__[self.pointorder[j]], own), self.Poker2Num(pok[0][0]+__cardscale__[self.pointorder[j]], own)+54])
                                return out
                        elif suc_flag:
                            tmp = 0
                            suc_flag = False
    ...
    def EndGame(self, banker, score):
        ...
        if banker_win:
            ...
            if new_lv_index >= len(__POINT__): new_lv_index = 0
    ...
    def parsePoker(self, poker, level):
    # poker: 甩牌牌型 list[int]
    # own: 各家持牌 list
    # level & major: 级牌、主花色
        pok = [self.num2Poker(p) for p in poker]
    ...
    # return -> move(list[int])
        ...
        if self.checkPokerType(standard_move, level) != "suspect": # 不是甩牌
            response = self.checkResP(standard_move, deck, level)
            if response:
                return response
            else:
                if standard_poker[0] in self.Major:
                    ...
                    if len(deck_Major) < len(standard_poker):
                        ...
                        target_parse = self.parsePoker(deck_Major, level)
        ...
        try:
            if self.step_count == 0: # 刚开局
                ...
                if self.step_count < 200: # 给下一名玩家发牌
                    ...
                    if newbanking["major"] == "":
                        newbanking["major"] = __suitset__[random.randint(0, __PLAYER_COUNT__)]
                        self.setMajor(newbanking["major"], self.globalInfo["level"])
                ...
                outid = []
                for pok in outpok:
                    id = self.Poker2Num(pok, curr_move)
                    outid.append(id)
                    curr_move.remove(id)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if stage == "deal":
            ...
            hold = env.getPlayerHoldCards(env.getPlayerPosition())
            response = [env.getPlayerPosition(), env.playCard(history_curr, hold, level)]
