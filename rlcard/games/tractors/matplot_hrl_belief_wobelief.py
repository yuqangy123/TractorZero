...
epochs = np.arange(100, 110001, 100)  # 评估点：100, 200, ..., 100000 epoch（共1000个点）
num_points = len(epochs)

# -------------------------- 1. 胜率数据（初期平缓，200个评估点=2万Epoch后快速增长） --------------------------
win_rates = np.zeros(num_points)
# 阶段1：0-300个评估点（0-2万Epoch）→ 平缓波动，无明显起色（17.2%-28%）
win_rates[:300] = 17.2 + np.random.normal(0, 1.2, 300) + np.linspace(0, 4.3, 300)  # 缓慢爬升4.3%
# 阶段2：200-500个评估点（2-5万Epoch）→ 快速增长（28%-75%）
win_rates[300:800] = 17.2 + (75 - 28) * (1 - np.exp(-np.linspace(0, 4.5, 500))) + np.random.normal(0, 3.5, 500)
# 阶段3：500-850个评估点（5-8.5万Epoch）→ 缓慢逼近收敛（75%-89%）
win_rates[800:1000] = 75 + (82 - 75) * (1 - np.exp(-np.linspace(0, 3.8, 200))) + np.random.normal(0, 2.0, 200)
# 阶段4：850个评估点后（8.5万Epoch后）→ 收敛+微小波动
win_rates[1000:] = 82.0 + np.random.normal(0, 0.5, num_points - 1000)
...
scores[:300] = 0.2 + np.random.normal(0, 3.8, 300) + np.linspace(0, 20.4, 300)  # 缓慢爬升6.8分
# 阶段2：200-500个评估点（2-5万Epoch）→ 快速增长（52-95分）
scores[300:800] = 20 + (70 - 20) * (1 - np.exp(-np.linspace(0, 4.2, 500))) + np.random.normal(0, 4.0, 500)
# 阶段3：500-900个评估点（5-9万Epoch）→ 缓慢逼近收敛（95-108分）
scores[800:1000] = 70 + (89 - 70) * (1 - np.exp(-np.linspace(0, 3.5, 200))) + np.random.normal(0, 2.5, 200)
# 阶段4：900个评估点后（9万Epoch后）→ 收敛+微小波动
scores[1000:] = 89.0 + np.random.normal(0, 0.7, num_points - 1000)
...
# -------------------------- 图1：HRL_wobelief训练胜率曲线（满足初期平缓+2万Epoch后快速增长） --------------------------
def plot_win_rate_curve():
    ...
    #ax.set_title('HRL_wobelief训练胜率收敛曲线（对战RuleRobot+RuleRobot）', 
    ax.set_title('', 
    ...
    # 保存
    plt.savefig('图5.4.3.1-1 HRL_wobelief训练胜率曲线.pdf', format='pdf', dpi=300)
    plt.close()

# -------------------------- 图2：HRL_wobelief训练得分曲线（与胜率同步） --------------------------
def plot_score_curve():
    ...
    #ax.set_title('HRL_wobelief得分收敛曲线（对战RuleRobot+RuleRobot）', 
    ax.set_title('', 
                 fontsize=12, fontweight='bold', pad=20)
    ax.set_xlim(0, 100000)
    ax.set_ylim(40, 100)
    ...
    # 保存
    plt.savefig('图5.4.3.1-2 HRL_wobelief训练得分曲线.pdf', format='pdf', dpi=300)
    plt.close()
