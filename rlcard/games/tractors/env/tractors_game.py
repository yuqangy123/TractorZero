...
class tractorsGame(Env):
    ...
    # 确定主牌
    def setMajor(self, major, level, repo_card=None, bid_seat=None):
        self.Major = copy.deepcopy(__MAJOR__)
        self.pointorder = copy.deepcopy(__POINT__)
        if major != 'n': # 非无主
        ...
        if bid_seat and repo_card:
            self.globalInfo["bid_history"].append([bid_seat, repo_card])
    ###############################################################
    # 报错模块
    # 报错类型：
    ...
    def setError(self, player, reason): # player: int-[0,3] reason: str
        ...
        self.erro_code = 1
        # print(json.dumps(self.errored[len(self.errored)-1]))
    ...
    #获取牌堆中所有符合牌型
    def getTypePoke(self, own_pok, level, type = ["single","pair","tractor", "suspect"], params=None):        
        rule_poks = []#poker格式
        ...
        if 'suspect' not in type:
            ...
            if 'tractor' in type:
                tr = self.parseTractorPoker(own_pok, level, params)
                rule_poks.extend(tr)
    ...
    #获取玩家手牌
    def getPlayerHoldCards(self, pos=None):
        if pos == None:
            return copy.deepcopy(self.curr_alloc)
    ...
    def getBidHistory(self):
        return self.globalInfo["bid_history"]

    #计算主牌牌力
    def calcMajorPokePower(self, own_poker, level, major):
        poker_deck = [self.num2Poker(id) for id in own_poker]
        major_pok = [pok for pok in poker_deck if \
                     pok[0]==major or pok[1]=='o' or pok[1]==level]
        #["single","pair","tractor", "suspect"]
        tractors = self.getTypePoke(major_pok, level, 'tractor', 'longest')

        major_pok = list(filter(lambda item: item not in tractors, major_pok))
        pairs = self.getTypePoke(major_pok, level, 'pair')

        major_pok = list(filter(lambda item: item not in pairs, major_pok))
        singles = self.getTypePoke(major_pok, level, 'single')

        power_score = 0
        for tractor in tractors:
            power_score += len(tractor)/2 * 2.5 + sum([t/10 for t in tractor])
        
        for pair in pairs:
            if pair[0] == 'j':power_score += 5
            elif pair[0] == 'J':power_score += 7
            elif pair[1] == level:power_score += 4
            elif pair[1] == '0' or pair[1] == 'K':power_score += 3
            else:power_score += 2

        for single in singles:
            if single[0] == 'j':power_score += 2
            elif pair[0] == 'J':power_score += 3
            elif pair[1] == level:power_score += 1.5
            elif pair[1] == '0' or pair[1] == 'K':power_score += 1
            else:power_score += 0.5
    ...
    #分析得到拖拉机牌型（大小王和级牌当然不会参与拖拉机）
    def parseTractorPoker(self, poker, level, params=None):
    # poker: 甩牌牌型 list[int]
        ...
        try:
            if self.step_count == 0: # 刚开局
                ...
                if len(repo) > 0:
                    ...
                    self.setMajor(newbanking["major"], self.globalInfo["level"], repo, currplayer)
