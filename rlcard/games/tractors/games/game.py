...
class tractorsGame():
    ...
    def checkResPerfect(self, play_pok, own_pok, level): # poker: list[int]
            ...
            # 主牌拖拉机             
                ...
                if len(ret) == 0:
                    ...
                    if len(pairspok) >= poker_len//2:
                        ...
                        for subpairs in combpairs:
                            sub_ret = []
                            for pairs in subpairs:
                                sub_ret.extend(pairs)
                            ret.append(sub_ret)
                            
                    #对子数不够，用对子+单牌组合
                    else:
                        singlepok = [p for p,v in my_pok_count.items() if v == 1]
                        combpairs = list(combinations(pairspok, poker_len//2 - len(pairspok)))
                        combpair_len = len(combpairs[0])*2 if len(combpairs)>0 else 0

                        #(单牌+对子)的数量<出牌数，则将所有的牌加进来
                        fixed_pok = []
                        if len(singlepok) + combpair_len < poker_len:
                            fixed_pok = singlepok
                            singlepok = [pok for pok in own_pok if pok not in deck_Major]
                        
                        if len(combpairs) == 0:
                            for pairs in pairspok:fixed_pok.extend(pairs)
                            ret = [fixed_pok + list(poks) for poks in combinations(singlepok, poker_len-len(fixed_pok))]
                            
                        else:
                            cut_len = poker_len - combpair_len - len(fixed_pok)
                            for pair in combpairs:
                                combsingle = list(combinations(singlepok, cut_len))
                                for single in combsingle:
                                    pairs =list(item for sublist in pair for item in sublist)
                                    ret.append(pairs+fixed_pok+list(single))
                return ret
                # pos = []
                # for k, v in my_pok_count.items():
                #     if v == 2:
                ...
                #                 out = []
                #                 for j in range(i+2):
                #                     p = self.Poker2Num(pok[0][0]+__CARDSCALE__[self.pointorder[j]], own)
                #                     out.extend([p, p+54])
                #                 return out
                #         elif suc_flag:
                #             tmp = 0
        ...
        #出的是副牌
            ...
            if poker_len <= 2:
                ...
                if len(suit_pok) < poker_len:
                    unsuit_pok = [pok for pok in own_pok if pok[0] != suit]
            ...
            #副牌拖拉机
                ...
                if len(ret) == 0:
                    ...
                    #对子数不够，用对子+单牌组合
                        ...
                        if len(singlepok) + combpair_len < poker_len:
                            fixed_pok = singlepok
                            singlepok = [pok for pok in own_pok if pok[0] != suit]
                ...
                #                     p = self.Poker2Num(pok[0][0]+__CARDSCALE__[self.pointorder[j]], own)
                #                     out.extend([p, p+54])
                #                     # out.extend([self.Poker2Num(suit+__CARDSCALE__[self.pointorder[j]], own), self.Poker2Num(suit+__CARDSCALE__[self.pointorder[j]], own)+54])
                #                 return out
                #         elif suc_flag:
                #             tmp = 0
                #             suc_flag = False
    ...
    def playCard(self, history, deck, level):
        
        poker_deck = [self.num2Poker(id) for id in deck]
        # print("playCard.deck", poker_deck, '\r\n', deck)
        ...
        #test code
        # poker_deck = ['d2', 'd2', 'd3', 'd3', 'd4', 'd4', 'd5', 'd5', 'd6', 'd6', 'd7', 'd7', 'd8', 'd8', 'd9', 'd0', 'd0', ]
        # mj = 'd' #self.globalInfo['banking']['major']
        # standard_poker = [mj+'3', mj+'3', mj+'4', mj+'4', mj+'5', mj+'6' ]
        # if False:
        ...
        if self.checkPokerType(standard_move, level) != "suspect": # 不是甩牌
            ...
            response = self.checkResPerfect(standard_poker, own_pok, level)
            if response:
                ...
                #出的是副牌
                    ...
                    if len(deck_suit) <= len(standard_poker):#副牌数量不够
                        ...
                        # target_parse = self.parsePoker(deck_suit, level)
                        # target_parse.sort(key=lambda x: len(x), reverse=True)
                        # target_len = len(standard_poker)
                        # out = []
                        # for poks in target_parse:
        ...
        #甩牌
            ...
            if standard_poker[0] in self.Major:
                ...
                #手上的主牌数量够，甩牌
                    ...
                    standard_pok_counter = Counter(standard_poker)
                    standard_pok_counter = sorted(standard_pok_counter.items(), key=lambda item: item[1], reverse=True)
                    #无对子的甩牌
                    if standard_pok_counter[0][1] == 1:
                        comb_poks = list(combinations(deck_Major, len(standard_poker)))
                        out = []
                        for comb in comb_poks:
                            out.append(list(comb))
                    #有对子的甩牌
                    else:
                        pair_count = 0
                        for k,v in standard_pok_counter:
                            if v == 2:
                                pair_count += 1
                            else:
                                break
                        ...
                        #分析可出手牌中对子和单牌的个数
                        pok_fix_pairs, pok_single = [], []
                        pok_counter = Counter(deck_Major)
                        pok_counter = sorted(pok_counter.items(), key=lambda item: item[1], reverse=True)
                        for k,v in pok_counter:
                            if v == 2:
                                pok_fix_pairs.append([k,k])
                            else:
                                pok_single.append(k)
                        
                        #手上的对子数量是否比甩牌中的对子多
                        if len(pok_fix_pairs) > pair_count:
                            pok_fix_pairs = list(combinations(pok_fix_pairs, pair_count))
                            tmp_pok_fix_pairs = []
                            for pok_fix_pair in pok_fix_pairs:
                                tmp = [p for pok_fix_pair in pok_fix_pair for p in pok_fix_pair]
                                tmp_pok_fix_pairs.append(tmp)
                            pok_fix_pairs = tmp_pok_fix_pairs

                            out = []
                            pok_single_counter = list(combinations(pok_single, len(standard_poker) - pair_count*2))
                            if len(pok_single_counter) == 0:
                                for pok_pair in pok_fix_pairs:
                                    pok_select = [pok for pok in deck_Major if pok not in pok_pair]
                                    pok_select_counter = list(combinations(pok_select, len(standard_poker) - len(pok_pair)))
                                    for p in pok_select_counter:
                                        out.append(pok_pair + list(p))
                            else:
                                for pok_pair in pok_fix_pairs:
                                    for pok_singles in pok_single_counter:
                                        out.append(pok_pair + list(pok_singles))

                        else:
                            pok_fix_pairs = [pok for pok_fix_pair in pok_fix_pairs for pok in pok_fix_pair]
                            out = []
                            pok_single_counter = list(combinations(pok_single, len(standard_poker) - len(pok_fix_pairs)))
                            for pok_singles in pok_single_counter:
                                out.append(pok_fix_pairs + list(pok_singles))
                    ...
                    # target_parse = self.parsePoker(standard_poker, level)
                    # deck_parse = self.parsePoker(deck_Major, level)                    
                    # target_parse.sort(key=lambda x: len(x), reverse=True)
                    # deck_parse.sort(key=lambda x: len(x), reverse=True)
                    # out = []
                    # for target_unit in target_parse:
                    #     target_unit_len = len(target_unit)
                    #     for deck_unit in deck_parse:
                    #         if len(deck_unit) >= target_unit_len:
                    #             out.extend(deck_unit[:target_unit_len])
                    #             target_unit_len = 0
                    #             deck_unit = deck_unit[target_unit_len+1:]
                    #             deck_parse.sort(key=lambda x: len(x), reverse=True)
                    #         else:
                    #             out.extend(deck_unit)
                    #             target_unit_len -= len(deck_unit)
                    #             deck_unit = []
                    #             deck_parse.sort(key=lambda x: len(x), reverse=True)
                    # resp = []
                    # _deck = deck + []
            ...
            #副牌甩牌
                ...
                #可出副牌数够
                    ...
                    #无对子的甩牌
                    if standard_pok_counter[0][1] == 1:
                        comb_poks = list(combinations(deck_suit, len(standard_poker)))
                        out = []
                        for comb in comb_poks:
                            out.append(list(comb))
                    #有对子的甩牌
                        ...
                        if len(pok_fix_pairs) > pair_count:
                            ...
                            if len(pok_single_counter) == 0:
                                for pok_pair in pok_fix_pairs:
                                    pok_select = [pok for pok in deck_suit if pok not in pok_pair]
                                    pok_select_counter = list(combinations(pok_select, len(standard_poker) - len(pok_pair)))
                                    for p in pok_select_counter:
                                        out.append(pok_pair + list(p))
        ...
        for poks in out:
            ...
            for pok in poks:
                if type(pok) == list:
                    self.playCard(history, deck, level)
                    pass
                cardid = self.Poker2Num(pok, _deck)
            ...
            if self.step_count == 0: # 刚开局
                ...
                self.logs.append({"output":{
                    ...
                    "display": { 
                        ...
                        "deliver": {
                            "player": first,
                            "poker": self.globalInfo["deliver"],
                        },
                        "publiccard": publiccard,
                        "level": self.globalInfo["level"]
                    },
                }})
                # print(json.dumps(self.logs[-1]))
                ...
                if self.step_count < 200: # 给下一名玩家发牌
                    ...
                    self.logs.append({"output":{
                        ...
                        "display":{
                            ...
                            "publiccard": self.globalInfo["publiccard"],
                        }
                    }})
                    # if new_:
                    #     printAll["output"]["display"]["banking"] = newbanking
                    ...
                    printAll = {"output":{
                        ...
                        "display": {
                            ...
                            "publiccard": self.globalInfo["deliver"],
                        }
                    }}
                    if new_:
                        printAll["output"]["display"]["banking"] = newbanking
                    self.logs.append(printAll)
...
def run(env):
    ...
    if stage == "finish":
        ...
        playedCards = env.playCard(history_curr, hold, level)
