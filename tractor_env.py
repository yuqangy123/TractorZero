...
class tractorGame():
    ...
    def PokerList2Num(self, pokers, deck): # poker: str        
        ...
        for _pokers in pokers:
            ...
            for poker in _pokers:
                ...
                p = NumInDeck if NumInDeck in deck else NumInDeck + 54
                rule_poker.append(p)
    ...
    def checkPokerType(self, poker, level): #poker: list[int]
        poker = [self.num2Poker(p) for p in poker]
        if len(poker) == 1:
    ...
    #获取牌堆中所有符合牌型
    def getTypePoke(self, own_pok, level, type = ["single","pair","tractor"]):        
        rule_poks = []#poker格式
        ...
        if 'tractor' in type:
            tr = self.parsePoker(own_pok, level, False)
            rule_poks.extend([t for t in tr if len(t)>=4])
    ...
    def checkResPerfect(self, poker, own, level): # poker: list[int]
        pok = [self.num2Poker(p) for p in poker]
        own_pok = [self.num2Poker(p) for p in own]
        poker_len = len(poker)
        if pok[0] in self.Major:#出的是主牌
            major_pok = [pok for pok in own_pok if pok in self.Major]
            my_pok_count = Counter(major_pok)            
            if poker_len <= 2:#单张或对子
                ret = []
                for k,v in my_pok_count.items():
                    if v >= poker_len:
                        p = self.Poker2Num(k, own)
                        if poker_len == 1:
                            ret.append([p])
                        else:
                            ret.append([p, p+54])
                ...
                tractors = self.parsePoker(deck_Major, level, False)
            ...
            if poker_len <= 2:#单张或对子
                ...
                #     if v >= len(poker):
                #         if len(poker) == 1:
                #             return [self.Poker2Num(k, own)]
                #         else:
                #             return [self.Poker2Num(k, own), self.Poker2Num(k,own) + 54]  
            else:#拖拉机
                deck_Major = [pok for pok in own_pok if pok in self.Major]
                tractors = self.parsePoker(deck_Major, level, False)
                ret_tr = [tractor for tractor in tractors if len(tractor) == poker_len]
                
                deck_nMajor = [pok for pok in own_pok if pok not in self.Major]
                tractors = self.parsePoker(deck_nMajor, level, False)
                ret_un_tr = [tractor for tractor in tractors if len(tractor) == poker_len]
                return ret_un_tr + ret_tr
                # pos = []
                # for k, v in my_pok_count.items():
                #     if v == 2:
                #         pos.append(self.pointorder.index(k[1]))
                ...
                if outhis[0][0] in self.Major: 
                    ...
                    if len(major_pok) != len(poker): # 这种情况下，同花(主牌)必须已经贴完
                        ...
                        if self.checkPokerType(outhis[0], level) == "tractor": # 拖拉机来喽
                            ...
                            own_divide, r = self.checkThrow(major_hold, [[]], currplayer, level, check=False)
                    ...
                    if len(suit_pok) != len(poker): # 这种情况下，同花(主牌)必须已经贴完
                        ...
                        if self.checkPokerType(outhis[0], level) == "tractor": # 拖拉机来喽
                            ...
                            divider.sort(key=lambda x: len(x), reverse=True)
                            dividcnt = [len(x) for x in divider]
                            own_divide, r = self.checkThrow(suit_hold, [[]], currplayer, level, check=False)
                            own_divide.sort(key=lambda x: len(x), reverse=True)
                            own_cnt = [len(x) for x in own_divide]
                            for poktype in outhis: # 可以使用这种方法的原因在于同一组花色/主牌可组成的牌型数量太少，不会出现多解
                                if dividcnt[0] >= len(poktype):
    ...
    #分析得到拖拉机牌型（大小王和级牌当然不会参与拖拉机；可选单牌）
    def parsePoker(self, poker, level, allType=True):
    # poker: 甩牌牌型 list[int]
    # own: 各家持牌 list
    # level & major: 级牌、主花色
        if len(poker) == 0:
            return []
        pok = poker if type(poker[0]) == str else [self.num2Poker(p) for p in poker]
        ...
        while i < len(pok):
            ...
            if (k != 'jo' and k != 'Jo' and k[1] != level):
                if suit == k[0] and last_pos + (j//2) == self.pointorder.index(k[1]):
                    ...
                    # if len(tractor) >= 4 and len(tractor)%2 == 0:
                    if len(tractor) >= 4 or (len(tractor) < 4 and allType == True):
                        tractor = tractor[:len(tractor)//2*2]
                        outpok.append(tractor)
                        tractor_list.extend(tractor)
        ...
        def find_long_subarrays(arr, min_length=6):
            subarrays = []
            n = len(arr)            
            for start in range(0, n - min_length + 1, 2):
                for end in range(start + min_length, n + 1, 2):
        ...
        for _opok in outpok :
            _opok_len = len(_opok)
            if _opok_len >= 6 and _opok_len%2 == 0:
                sub_tractor = find_long_subarrays(_opok[:], 4)
    ...
    def playCard(self, history, deck, level):
        ...
        #test code
        # poker_deck = ['d2', 'd2', 'd3', 'd3', 'd4', 'd4', 'd5', 'd5', 'd6', 'd6', 'd7', 'd7', 'd8', 'd8', 'd9', 'd0', 'd0', ]#test code
        # mj = self.globalInfo['banking']['major']
        # standard_poker = [mj+'3', mj+'3', mj+'4', mj+'4' ]
        # poker_deck = [mj+'9', mj+'0', mj+'9', mj+'0']
        
        #print(checkPokerType(standard_move, level))
        if self.checkPokerType(standard_move, level) != "suspect": # 不是甩牌
            response = self.checkResPerfect(standard_move, deck, level)
            if response:
            # if False:#test code
                return response
            else:
                ...
                if standard_poker[0] in self.Major:
                    ...
                    #手上主牌够，必须出主牌
                        ...
                        #拖拉机
                            ...
                            target_parse = self.parsePoker(deck_Major, level, False)
                ...
                #出的是副牌
                    ...
                    if len(deck_suit) < len(standard_poker):#副牌数量不够
                        my_nmajor = copy.deepcopy(deck_suit)
                        deck_nMajor = [pok for pok in poker_deck if pok not in deck_suit]
                        comb_nMajor = list(combinations(deck_nMajor, len(standard_poker) - len(deck_suit)))
                        out = []
                        for comb in comb_nMajor:
                            out.append(my_nmajor + list(comb))
                        ...
                        if target_len == 1:#单牌
                            ...
                            target_parse = self.parsePoker(deck_suit, level, False)
        ...
        #甩牌
            ...
            if standard_poker[0] in self.Major:
                ...
                #手上的主牌数量够，甩牌
                    ...
                    target_parse = self.parsePoker(standard_move, level)
                ...
                if len(deck_suit) < len(standard_poker):
                    ...
                    deck_parse = self.parsePoker(deck_suit, level)
                    target_parse = self.parsePoker(standard_move, level)
                    deck_parse.sort(key=lambda x: len(x), reverse=True)
                    target_parse.sort(key=lambda x: len(x), reverse=True)
                    out = []
        ...
        for poks in out:
            ...
            for pok in poks:
                ...
                if cardid not in _deck:
                    print(self.Pokers2Num(poks,_deck))#test code
                    self.playCard(history, deck, level)#test code
                _deck.remove(cardid)
                poksNumber.append(cardid)
            ret.append(poksNumber)
        return ret
            ...
            if self.step_count == 0: # 刚开局
                ...
                for pok in curr_move:
                    if pok not in self.curr_alloc[currplayer]:
                        self.setError(currplayer, "NOT_YOUR_POKER")
                outpok = self.checkLegalMove(curr_move, self.globalInfo["level"], major, currplayer, history[1], self.curr_alloc, banker)
                # collect history
                outid = []
                for pok in outpok:
