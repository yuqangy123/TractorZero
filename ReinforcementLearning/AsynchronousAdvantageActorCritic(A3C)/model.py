...
class Net(nn.Module):
    ...
    def choose_action(self, s):
        ...
        ac = t.argmax(prob).numpy()
        # return ac
        
        #根据概率随机采样。相当于增加探索
        m = self.distribution(prob)         
        # if ac.item() != m.sample().numpy()[0].item():
        #     print(ac.item(), m.sample().numpy()[0].item())
...
class Worker(mp.Process):
    def __init__(self, gnet, opt, global_ep, global_ep_r, res_queue, name, n_features, n_actions):
        ...
        self.env = gym.make('CartPole-v0', render_mode= 'human' if self.name == "w00" else None).unwrapped
        self.MAX_EP = 30000
